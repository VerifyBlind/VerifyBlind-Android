...
class CryptoUtilsTest {
    ...
    // DEK rastgeledir ve personId'den TÜRETİLEMEZ → ham anahtar alan varyant gerekir.
    // Yukarıdaki personId'li metotlar v1 yedek formatı için değişmeden kalır.

    @Test
    fun aesGcmRaw_roundTrip_succeeds() {
        val key = ByteArray(32) { it.toByte() }
        val data = "KEK/DEK round-trip — Türkçe 😀"
        val (enc, iv) = CryptoUtils.aesGcmEncryptRaw(data, key)
        assertEquals(data, CryptoUtils.aesGcmDecryptRaw(enc, iv, key))
    }

    @Test
    fun aesGcmRaw_wrongKey_throwsException() {
        val key = ByteArray(32) { it.toByte() }
        val wrongKey = ByteArray(32) { (it + 1).toByte() }
        val (enc, iv) = CryptoUtils.aesGcmEncryptRaw("gizli", key)
        try {
            CryptoUtils.aesGcmDecryptRaw(enc, iv, wrongKey)
            fail("Yanlış ham anahtar ile şifre çözme başarısız olmalıydı")
        } catch (e: Exception) {
            // Bekleniyor: GCM authentication tag mismatch
        }
    }
    ...
    @Test
    fun aesGcmRaw_differentIvEachCall() {
        val key = ByteArray(32) { it.toByte() }
        val (_, iv1) = CryptoUtils.aesGcmEncryptRaw("aynı veri", key)
        val (_, iv2) = CryptoUtils.aesGcmEncryptRaw("aynı veri", key)
        assertNotEquals("GCM IV her çağrıda farklı olmalı", iv1, iv2)
    }

    @Test
    fun kekFromPersonId_matchesDeriveKeyFromPersonId() {
        // KEK, bugünkü bulut anahtarıyla BİREBİR aynı bayt olmalı: v1→v2 geçişinde
        // mevcut personId'ler wrap'leri açabilsin diye.
        val pid = "person-abc-xyz-789"
        assertArrayEquals(
            CryptoUtils.deriveKeyFromPersonId(pid).encoded,
            CryptoUtils.kekFromPersonId(pid)
        )
    }

    @Test
    fun generateDek_is32BytesAndRandom() {
        val a = CryptoUtils.generateDek()
        val b = CryptoUtils.generateDek()
        assertEquals(32, a.size)
        assertEquals(32, b.size)
        assertFalse("İki DEK aynı olmamalı (rastgelelik)", a.contentEquals(b))
    }

    @Test
    fun generateDek_canWrapAndUnwrapWithKek() {
        // Uçtan uca: DEK üret → KEK ile sar → aç → aynı DEK.
        val dek = CryptoUtils.generateDek()
        val kek = CryptoUtils.kekFromPersonId("person-1")
        val dekB64 = Base64.encodeToString(dek, Base64.NO_WRAP)

        val (enc, iv) = CryptoUtils.aesGcmEncryptRaw(dekB64, kek)
        val opened = Base64.decode(CryptoUtils.aesGcmDecryptRaw(enc, iv, kek), Base64.DEFAULT)
