...
class HistoryAdapter : ListAdapter<HistoryEntity, RecyclerView.ViewHolder>(HistoryDiffCallback()) {
    ...
    // Listede birden fazla farklı cihaz varsa satırlarda cihaz adı gösterilir.
    ...
    fun setShowDevice(show: Boolean) {
        if (showDevice == show) return
        showDevice = show
        notifyDataSetChanged()
    }
    ...
    inner class HistoryViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: HistoryEntity) {
            ...
            // Cihaz adı: yalnız birden fazla cihaz varsa VE bu kaydın adı biliniyorsa göster.
            if (showDevice && item.deviceName.isNotBlank()) {
