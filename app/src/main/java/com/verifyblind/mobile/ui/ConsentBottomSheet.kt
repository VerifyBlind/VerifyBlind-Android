...
class ConsentBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Scope maddeleri — user_id ilk sırada ve "Bu nedir?" linki HEMEN onun bitişinde
        // (en altta değil). Detay, kalıcı tanıma kodunun ne olduğunu sade dille açıklar.
        val obj = partnerInfo.validations?.takeIf { it.isJsonObject }?.asJsonObject
        var anyItem = false
        if (obj != null) {
            if (obj.has("user_id")) {
                addScopeItem(title = requireContext().getString(R.string.scope_user_id))
                addWhatIsCodeLink()
                anyItem = true
            }
            if (obj.has("age")) {
                addScopeItem(title = requireContext().getString(R.string.scope_age, obj.get("age").asString))
                anyItem = true
            }
        }
        if (!anyItem) addScopeItem(title = requireContext().getString(R.string.consent_default_scope))
    ...
    private fun addWhatIsCodeLink() {
        val ctx = requireContext()
        val dp = resources.displayMetrics.density

        val label = getString(R.string.scope_user_id_whatis)
        ...
        val tv = TextView(ctx).apply {
            ...
            // Sol girinti: "• " genişliği kadar — link, user_id maddesinin metniyle hizalı görünsün.
            setPadding((14 * dp).toInt(), (2 * dp).toInt(), 0, (6 * dp).toInt())
            ...
            setOnClickListener {
                showInfoDialog(
                    title = getString(R.string.scope_user_id_detail_title),
                    content = getString(R.string.scope_user_id_detail_body)
                )
            }
        }
        binding.layoutScopeItems.addView(tv)
    ...
    private fun showPrivacyNoticeDialog(content: String) =
        showInfoDialog(title = getString(R.string.privacy_notice_title), content = content)
