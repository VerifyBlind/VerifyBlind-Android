...
class DropboxProvider(private val context: Context) : CloudProvider {
    ...
    override suspend fun list(suffix: String): Result<List<CloudFileEntry>> = withContext(Dispatchers.IO) {
        try {
            ...
            var res = client.files().listFolder("") // app-scoped kök
            while (true) {
                res.entries
                    .filterIsInstance<com.dropbox.core.v2.files.FileMetadata>()
                    .filter { it.name.endsWith(suffix, ignoreCase = true) }
                    .forEach { entries.add(CloudFileEntry(it.name, it.serverModified?.time)) }
                if (!res.hasMore) break
                res = client.files().listFolderContinue(res.cursor)
            }
            Result.success(entries.sortedByDescending { it.modifiedAtMillis ?: 0L })
        } catch (e: com.dropbox.core.v2.files.ListFolderErrorException) {
