...
enum class AttestFailureKind { AUTHORIZATION, INTEGRITY }
...
object AttestationVerifier {
    ...
    data class VerificationResult(
        ...
        // Konvansiyon: null failureKind (etiketlenmemiş başarısızlık) mesaj eşleyicilerince INTEGRITY
        // sayılır (güvenli taraf). Yalnız AUTHORIZATION site'ları açıkça etiketlenir.
        val failureKind: AttestFailureKind? = null
    )
    ...
    fun verify(
        ...
        return try {
            ...
            verifyRealDocument(cbor, pcr0Signature)
        } catch (e: Exception) {
            AppLog.error("Tasdik doğrulama istisnası: ${e.message}", TAG, e)
    ...
    private fun verifyRealDocument(
        ...
        // PCR0 tümü sıfır = enclave debug-mode (geçersiz/güvenilmez ölçüm). HER ZAMAN reddedilir.
        if (pcr0 != "UNKNOWN" && pcr0.all { it == '0' }) {
            AppLog.error("[Tasdik] ❌ Enclave debug-mode (PCR0 tümü sıfır) — reddedildi.", TAG)
            return VerificationResult(false, "Enclave started in debug-mode (PCR0 is all zeros) — rejected.")
        }

        // =
        // KONTROL 1b: COSE_Sign1 İmza Doğrulaması (payload ↔ donanım bağı)
        // Sertifika zinciri (Kontrol 1) leaf'in GÜVENİLİR olduğunu kanıtlar; bu imza ise payload'ın
        // (user_data + PCR0 dahil) o leaf'in private key'iyle imzalandığını kanıtlar. Bu kontrol
        // olmadan saldırgan gerçek bir belgenin zincirini/PCR0'ını/imzasını koruyup user_data'ya
        // kendi RSA anahtarını koyabilir (diğer 3 kontrol geçer) → TCKN hasadı.
        // Dev/mock yolu (PCR0 sıfır) yukarıda erken döndüğü için buraya yalnız gerçek donanım
        // belgeleri ulaşır.
        // =
        val coseSigResult = verifyCoseSignature(cose, payload, payloadBytes)
        if (!coseSigResult.first) {
            AppLog.error("[Tasdik] ❌ COSE_Sign1 imza doğrulaması BAŞARISIZ: ${coseSigResult.second}", TAG)
            return VerificationResult(false, "COSE signature check FAILED: ${coseSigResult.second}")
        }
        Log.i(TAG, "[Tasdik] ✅ Kontrol 1b: COSE_Sign1 imzası leaf sertifikayla doğrulandı.")
        ...
        if (!pcr0Signature.isNullOrBlank()) {
            ...
            if (authCheck.first) {
                ...
                AppLog.error("[Tasdik] ❌ PCR0 Yetkilendirme BAŞARISIZ: ${authCheck.second}", TAG)
                return VerificationResult(false, "PCR0 Authorization FAILED: ${authCheck.second}", failureKind = AttestFailureKind.AUTHORIZATION)
            }
        }
        else {
            AppLog.error("[Tasdik] ❌ Yetkisiz Enclave: İmza sağlanmadı.", TAG)
            return VerificationResult(false, "Unauthorized Enclave: No PCR0 signature provided.", failureKind = AttestFailureKind.AUTHORIZATION)
        }
...
-----END CERTIFICATE-----
    ...
    private fun verifyCertificateChain(payload: CBORObject): Pair<Boolean, String> {
        return try {
            ...
            AppLog.error("Zincir doğrulama hatası: ${e.message}$detail", TAG, e)
    ...
    private fun parseRsaCspBlob(blob: ByteArray): java.security.PublicKey? {
        return try {
            ...
            AppLog.error("CSP Blob ayrıştırma hatası: ${e.message}", TAG, e)
            null
        }
    }
    ...
    private fun extractEnclavePublicKey(payload: CBORObject): String? {
        return try {
            ...
            AppLog.error("Enclave Genel Anahtarı çıkarılamadı: ${e.message}", TAG, e)
            null
        }
    }

    //
    // Kontrol 1b: COSE_Sign1 İmza Doğrulaması
    // AWS Nitro belgesi ES384 (ECDSA P-384, SHA-384) ile imzalanır. İmza, leaf sertifikanın
    // public key'iyle RFC 8152 §4.4 Sig_structure üzerinden doğrulanır:
    //   Sig_structure = ["Signature1", protected_header(bstr), external_aad(boş bstr), payload(bstr)]
    //
    private fun verifyCoseSignature(cose: CBORObject, payload: CBORObject, payloadBytes: ByteArray): Pair<Boolean, String> {
        return try {
            val protectedBytes = cose[0].GetByteString()
            val signatureRaw = cose[3].GetByteString()  // ham r‖s (P-384 → 96 bayt)

            val certObj = payload["certificate"] ?: return Pair(false, "Missing leaf certificate")
            val factory = CertificateFactory.getInstance("X.509")
            val leafCert = factory.generateCertificate(certObj.GetByteString().inputStream()) as X509Certificate

            // RFC 8152 §4.4 Sig_structure — sıra ve tipler birebir COSE_Sign1 imzasıyla eşleşmeli.
            val sigStructure = CBORObject.NewArray().apply {
                Add("Signature1")          // context (text string)
                Add(protectedBytes)        // body_protected (bstr — serileştirilmiş protected header)
                Add(ByteArray(0))          // external_aad (boş bstr)
                Add(payloadBytes)          // payload (bstr)
            }
            val toBeSigned = sigStructure.EncodeToBytes()

            // COSE imzası ham r‖s; Java/BouncyCastle ECDSA verify ASN.1 DER bekler → çevir.
            val derSig = ecRawToDer(signatureRaw)

            val verifier = Signature.getInstance("SHA384withECDSA", BouncyCastleProvider.PROVIDER_NAME)
            verifier.initVerify(leafCert.publicKey)
            verifier.update(toBeSigned)

            if (verifier.verify(derSig)) Pair(true, "OK")
            else Pair(false, "COSE_Sign1 signature mismatch")
        } catch (e: Exception) {
            Pair(false, "COSE verify error: ${e.message}")
        }
    }

    /** Ham ECDSA imzası (r‖s, sabit boy) → ASN.1 DER SEQUENCE{INTEGER r, INTEGER s}. */
    private fun ecRawToDer(raw: ByteArray): ByteArray {
        val half = raw.size / 2
